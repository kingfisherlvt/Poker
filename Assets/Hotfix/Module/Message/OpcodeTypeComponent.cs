...
namespace ETHotfix
{
	...
	public class OpcodeTypeComponent : Component
	{
		...
		{
            object message = this.typeMessages[opcode];
            ResetInstance(message);
            return message;
		}

        public void ResetInstance(object obj)
        {
            foreach (System.Reflection.PropertyInfo p in obj.GetType().GetProperties())
            {
                p.SetValue(obj, p.PropertyType.IsValueType ? Activator.CreateInstance(p.PropertyType) : null);
            }
        }
