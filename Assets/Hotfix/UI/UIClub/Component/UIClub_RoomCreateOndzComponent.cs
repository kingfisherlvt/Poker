...
{
    [ObjectSystem]
    public class UIClub_RoomCreateOndzComponentAwakeSystem : AwakeSystem<UIClub_RoomCreateOndzComponent>
    {
        public override void Awake(UIClub_RoomCreateOndzComponent self)
        {
            self.Awake();
        }
    }

    public class UIClub_RoomCreateOndzComponent : UIBaseComponent
    {
        ...
        public void Awake()
        {
            //默认当前界面内容
            isInit = false;
            rc = this.GetParent<UI>().GameObject.transform.Find("roomcreat_page2").GetComponent<ReferenceCollector>();
