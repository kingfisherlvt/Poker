...
{
    [ObjectSystem]
    public class UIClub_RoomCreateAofomaComponentSystem : AwakeSystem<UIClub_RoomCreateAofomaComponent>
    {
        public override void Awake(UIClub_RoomCreateAofomaComponent self)
        {
            self.Awake();
        }
    }

    public class UIClub_RoomCreateAofomaComponent : UIBaseComponent
    {
        public ReferenceCollector rc;
        int[] sizeblindArr = new int[] { 1, 2, 5, 10, 20, 25, 50, 100, 200, 300, 500, 1000 };
        int[] draginArr = new int[] { 8, 50, 100, 200, 300, 400 };
        int[] maxPlayTimeType = new int[] { 30, 60, 90, 120, 150, 180, 240, 360 };
        float[] qianzhuType = new float[] { 0 , 2 , 5 , 10 , 20 , 30 };
        /// <summary>
        /// 最短上桌时间
        /// </summary>
        float[] shortTimeArr;
        ...
        public void Awake()
        {
            //默认当前界面内容
            isInit = false;
            rc = this.GetParent<UI>().GameObject.transform.Find("roomcreat_page5").GetComponent<ReferenceCollector>();
        ...
        public override void OnShow(object obj)
        {
            ...
            if (!isInit)
            {
                ...
                assitent = rc.Get<GameObject>(UIClubAssitent.roomcreat_1_toggle_xuezhan);
                element_xuezhan = new ToggleElement(assitent.GetComponent<Toggle>(), assitent.transform.GetChild(1).gameObject.GetComponent<Text>());
            ...
            UIEventListener.Get(rc.Get<GameObject>(UIClubAssitent.roomcreat_btn_xztip)).onClick = (go) =>
            {
                Game.EventSystem.Run(UIClubEventIdType.CLUB_ROOMCREAT_TIP, 0);
            };

            UIEventListener.Get(rc.Get<GameObject>(UIClubAssitent.roomcreat_0_btn_qus)).onClick = (go) =>
            {
                Game.EventSystem.Run(UIClubEventIdType.CLUB_ROOMCREAT_TIP, 2);
            };

            //显示默认配置
            element_sizeblind.bar.numberOfSteps = sizeblindArr.Length;
            element_toggle6.toggle.gameObject.SetActive(false);//aof 统一去掉muck
            element_xuezhan.toggle.gameObject.SetActive(false);//aof  没有血战模式
        ...
        public void OnStart(int ctype)
        {
            ...
            //UIComponent.Instance.Toast("功能未开放");
            ...
            //新接口
            WEB2_room_omaha_create.RequestData omh = new WEB2_room_omaha_create.RequestData()
            {
            ...
            isClickCreat = true;
            HttpRequestComponent.Instance.Send(WEB2_room_omaha_create.API, WEB2_room_omaha_create.Request(omh), this.RoomCreatCall , null , this.RoomCreatError);
        }

        void RoomCreatCall(string resData)
        {
            isClickCreat = false;
            WEB2_room_omaha_create.ResponseData res_roomData = WEB2_room_omaha_create.Response(resData);
            if (res_roomData.status == 0)
            {
                //保存配置
                SaveConfig();
                ...
                GameCache.Instance.roomName = res_roomData.data.name;
                GameCache.Instance.roomIP = res_roomData.data.gmip;
                GameCache.Instance.roomPort = int.Parse(res_roomData.data.gmprt);
                GameCache.Instance.room_path = res_roomData.data.rpath;
                GameCache.Instance.room_id = res_roomData.data.rmid;
                GameCache.Instance.client_ip = res_roomData.data.cliip;
                GameCache.Instance.carry_small = res_roomData.data.incp;
                GameCache.Instance.straddle = res_roomData.data.sdlon;
                GameCache.Instance.insurance = res_roomData.data.isron;
                GameCache.Instance.muck_switch = res_roomData.data.muckon;
                GameCache.Instance.jackPot_fund = res_roomData.data.jpfnd;
                GameCache.Instance.jackPot_on = res_roomData.data.jpon;
                GameCache.Instance.jackPot_id = res_roomData.data.jpid;
                GameCache.Instance.shortest_time = res_roomData.data.gmxt;
                GameCache.Instance.rtype = res_roomData.data.rtype;
            ...
            {
                Log.Debug($"创建失败 status =  {res_roomData.status}");
                UIComponent.Instance.Toast(res_roomData.status);
            }
        ...
        void UpdatePlayTimeInfo(bool isEvent)
        {
            ...
            //大于6人时 不能开启保险模式
            if (element_people.step > 4)
            {

                element_toggle5.toggle.isOn = false;
                element_toggle5.toggle.interactable = false;
            }
            else element_toggle5.toggle.interactable = true;
        ...
        void SetDefaultConfig()
        {
            ...
            if (GameCache.Instance.reconfigs[crt] == null)
            {
                GameCache.Instance.reconfigs[crt] = new RcConfig()
                {
                    ...
                    element_dragin_value = 0.4f,
                    element_playtime_value = 0,
                    element_shortplaytime_value = 0,
                    element_thinktime_value = 0.5f,
            ...
            element_xuezhan.toggle.isOn = GameCache.Instance.reconfigs[crt].element_toggle_xuezhan_value;
        }
        ...
        void SaveConfig()
        {
            ...
            GameCache.Instance.reconfigs[crt].element_toggle_xuezhan_value = element_xuezhan.toggle.isOn;
        }
        #endregion
