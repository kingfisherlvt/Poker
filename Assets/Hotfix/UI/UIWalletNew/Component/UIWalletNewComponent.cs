...
{
    [ObjectSystem]
    public class UIWalletComponentSystem : AwakeSystem<UIWalletNewComponent>
    {
        public override void Awake(UIWalletNewComponent self)
        {
            self.Awake();
        }
    }

    public class UIWalletNewComponent : UIBaseComponent
    {
        ...
        protected virtual void InitUI()
        {
            ...
            UIEventListener.Get(rc.Get<GameObject>("BtnMyRecharge")).onClick = go =>
            {
                UIMineModel.mInstance.APIGetClubId(tDtoHasClub =>
                {
                    if (tDtoHasClub)
                    {
                        UIMineModel.mInstance.APIClubTransType(pDto => { UIComponent.Instance.ShowNoAnimation(UIType.UIMine_WalletAddBeansList, pDto); });
                    }
                    else
                    {
            ...
            //记录
            UIEventListener.Get(rc.Get<GameObject>("BtnCashFlow")).onClick = go => { UIComponent.Instance.ShowNoAnimation(UIType.UIMine_WalletFlow); };
        ...
        {
            UIMineModel.mInstance.ObtainUserInfo(pDto => { ShowWalletNum(); });
        }
