...
#else
            File.Copy(Path.Combine(ScriptAssembliesDir, HotfixDll).Replace('\\', '/'), Path.Combine(CodeDir, "Hotfix.dll.bytes").Replace('\\', '/'), true);
            File.Copy(Path.Combine(ScriptAssembliesDir, HotfixDll).Replace('\\', '/'), Path.Combine(CodeDir, "Hotfix.pdb.bytes").Replace('\\', '/'), true);
            //File.Copy(Path.Combine(ScriptAssembliesDir, HotfixMdb).Replace('\\', '/'), Path.Combine(CodeDir, "Hotfix.mdb.bytes").Replace('\\', '/'), true);
            Log.Info($"mono copy Hotfix.dll Hotfix.pdb to Res/Code complete");
#endif
