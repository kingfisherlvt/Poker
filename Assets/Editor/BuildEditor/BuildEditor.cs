...
namespace ETEditor
{
    ...
    public class BuildEditor : EditorWindow
    {
        ...
        [MenuItem("Tools/Packaging tool %w")]
        ...
        private void OnGUI()
        {
            ...
            this.isBuildExe = EditorGUILayout.Toggle("Whether to package EXE: ", this.isBuildExe);
            this.isContainAB = EditorGUILayout.Toggle("Do you want to import the resource into EXE?: ", this.isContainAB);
            ...
            this.buildAssetBundleOptions = (BuildAssetBundleOptions)EditorGUILayout.EnumFlagsField("BuildAssetBundleOptions(Multiple selections possible): ", this.buildAssetBundleOptions);

            if (GUILayout.Button("Clear all hot subcontracting AB marks"))
            {
                if (this.platformType == PlatformType.None)
                {
                    return;
                }
                EditorUtility.DisplayProgressBar("Clearing all tags", "...", 0);
                ClearPackingTagAndAssetBundle();
                EditorUtility.ClearProgressBar();
            }

            if (GUILayout.Button("***** ====> Just one click to set the mark"))
            {
                if (this.platformType == PlatformType.None)
                {
                    return;
                }
                EditorUtility.DisplayProgressBar("One-click setting mark", "...", 0);
            ...
            if (GUILayout.Button("Start Packaging", GUILayout.Height(40)))
            {
                if (this.platformType == PlatformType.None)
                {
                    Log.Error("Please select a packaging platform!");
                    return;
                }
            ...
            if (GUILayout.Button("Start packaging (encryption)", GUILayout.Height(40)))
            {
                if (this.platformType == PlatformType.None)
                {
                    Log.Error("Please select a packaging platform");
        ...
        private void SetPackingTagAndAssetBundle()
        {
            ...
            SetBundleByDir("Assets/Bundles/Emoji");
            SetBundleByDir("Assets/Bundles/Animoji");
        ...
        // 根据目录设置bundle
        private static void SetBundleByDir(string dir)
        {
            List<string> paths = EditorResHelper.GetPrefabsAndScenes(dir);
            foreach (string path in paths)
            {
                ...
                string bunldeName = path1.Replace("Assets/Bundles/", "");
                var fileName = Path.GetFileName(bunldeName);
                bunldeName = bunldeName.Replace("/" + fileName, "");
                bunldeName = bunldeName.ToLower();
                SetBundle(path1, bunldeName);
            }
        }
