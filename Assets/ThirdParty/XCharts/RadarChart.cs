...
namespace xcharts
{
    ...
    public class RadarChart : BaseChart
    {
        ...
        public void SetRadarData(RadarData[] datas)
        {
            SetRadarData(datas,true);
        }
        /// <summary>
        /// 设值
        /// </summary>
        /// <param name="pIsNameValue">名/值  是否显示</param>
        public void SetRadarData(RadarData[] datas, bool pIsNameValue)
        {
            List<RadarIndicator> indicators = new List<RadarIndicator>();
            seriesList.Clear();
            Series series = new Series
            {
                legendKey = "",
                showDataNumber = 0,
                dataList = new List<SeriesData>(),
            };
            foreach (RadarData data in datas)
            {
                RadarIndicator indicator = new RadarIndicator
                {
                    name = pIsNameValue ? $"<color=#E9BF80FF>{data.name}\n{data.value}%</color>" : "",
                    max = 0
                };
                indicators.Add(indicator);

                series.AddData("", data.value);
            }
            radarInfo.indicatorList = indicators;
